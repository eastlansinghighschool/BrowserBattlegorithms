# Guided Level Dossier: Level 30: Index Jobs
...
- project: `team-strategy-script step 2`

## Curriculum Row
- focus: Index jobs
- new vocabulary: different ally roles
- new Blockly: index comparison
- assumptions: Level 29
...
## Lesson Copy
- objective: Use runner index comparisons so one ally attacks and the other patrols upward.
- intro: The shared script is getting a job system. One ally is already pushing deep into enemy territory. Have that runner move out of the way, and send the other runner to score.
- tips: not found
- tutorial steps:
  - 1. Compare The Index (level-28-index-compare)
    - body: You can compare runner index to a number to choose different branches for different allies. That is how the shared script starts assigning jobs.
    - demo Blockly: not found
  - 2. Attacker And Patrol (level-28-jobs)
    - body: Each index value can be assigned a different role. Think about which ally is better positioned for the scoring job, and what the other should do to stay out of the way.
    - demo Blockly: not found
...
- objective: runner runner_1_AI_AllyP1_2 reaches enemy flag at (10, 5)
...
- player runner 1 (runner_1_AI_AllyP1) slot ally 0 at (7, 5); control ally; frozen no; enemy flag no
  - play direction: 1; home side: left; can jump: yes; can place barrier: no
...
- Project step fixture 2: `tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-02.xml`
  ...
  - distinct block types: 7 types: `battlegorithms_if_boolean_else`, `battlegorithms_move_toward`, `battlegorithms_move_up_screen`, `battlegorithms_on_each_turn`, `battlegorithms_value_compare`, `battlegorithms_value_number`, `battlegorithms_value_runner_index`
...
## Facts Only
- shared workspace project team-strategy-script step 2
- concept focus present: Index jobs
- starter XML contains 1 blocks
- project fixture XML present (2 files)
- toolbox exposes 48 authored block types

## Validation Pointers
- Readiness command: npm run level:readiness -- --level index-jobs --json
- Linter command: npm run lint:levels
- Project step fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-02.xml
- Project final fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/final.xml
