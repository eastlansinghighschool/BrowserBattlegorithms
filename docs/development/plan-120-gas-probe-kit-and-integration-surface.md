...
status: complete
resolution: "Probe kit delivered and orchestrator-verified over six review rounds. Build is plan-120-v2 across the public child, nested-frame shell, and tenant-identity page: repaired DOMTokenList handshake handling, context-gated storage classification that reports unknown rather than partitioned on an uncertain pairing, deidentified PLAN120_RESULT email/chat handoff with coarse browser/OS metadata, origin readings exported as comparison status rather than value (ratified as a reusable Stage 1 pattern), the integrations/google-apps-script/ surface, and a hygiene guardrail test asserting the three version literals agree. repair-01 closed a report-completeness gap in the advisor disposition record; repair-02 restored version-as-provenance after a post-completion behavior change; repair-03 fixed a shell status race in which the iframe load handler overwrote its own handshake-success banner after the timeout had already been cleared, leaving a valid run displaying 'waiting' forever. repair-03 is display-only — three lines of guard logic, no protocol or measurement change — so plan-120-v2 was deliberately kept and the version-as-provenance rule was given a stated, bounded exemption. The child's message-origin row is documented in both the probe README and the operator directions as the authoritative handshake indicator. First live Gate 1 reading confirmed the round trip works and produced the first observed HtmlService parent origin; stability across the four reading conditions remains unmeasured and is the finding that decides Stage 1's protocol size. plan-120-v1 result blocks remain invalid. 595/595 unit tests, 6/6 hygiene, clean build. Running the probes stays an owner action outside this packet; Gate 1 and Gate 2 remain open in docs/open-questions.md."
depends_on: []
