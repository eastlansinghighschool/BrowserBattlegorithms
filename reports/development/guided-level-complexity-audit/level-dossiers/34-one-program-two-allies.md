# Guided Level Dossier: Level 29: One Program, Two Allies
...
- project: `team-strategy-script step 1`

## Curriculum Row
- focus: One program, two allies
- new vocabulary: shared program for allies
- new Blockly: runner index
...
## Lesson Copy
- objective: Two allies now share one workspace. Use runner index so one ally attacks and the other supports.
- intro: This is the beginning of Team Strategy Script.  You want the ally with runner index 0 to be doing the scoring in this level, not the ally with runner index 1. The same program runs on both allies, so runner index has to decide which one takes the scoring job.
- tips: not found
- tutorial steps:
  - 1. One Workspace, Two Allies (level-27-shared-program)
    - body: Both allies run the same blocks every turn. The first ally has index 0 and the second has index 1. A check like "if runner index equals 0" means only the first ally follows that branch — the second skips it and does something else instead.
    - demo Blockly: not found
  - 2. Index 0 And Index 1 (level-27-index)
    - body: Only one ally should take the scoring job here. The other ally needs to stay clear of the lane so the shared script stays readable.
    - demo Blockly: not found
...
- Project step fixture 1: `tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-01.xml`
...
## Facts Only
- shared workspace project team-strategy-script step 1
- concept focus present: One program, two allies
- starter XML contains 1 blocks
- project fixture XML present (2 files)
- toolbox exposes 48 authored block types

## Validation Pointers
- Readiness command: npm run level:readiness -- --level one-program-two-allies --json
- Linter command: npm run lint:levels
- Project step fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-01.xml
- Project final fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/final.xml
