...
status: complete
resolution: "Four per-attempt counters delivered and orchestrator-verified: runnerCollisionBounces and mapBlockageBounces split on the existing runner.blockedOrBounced reason, resourceUnavailableAttempts from resource.unavailable, and ineffectiveFreezeUses from an Area Freeze that resolves having frozen nobody. All four are derived passively inside emit(), so bounded-window event-log eviction cannot undercount them, and all four are scoped to runnerRole === 'ally' after an empirical audit found human-controlled runners never appear in the scrimmage, resource, or multi-runner levels these criteria target. The four preflight corrections all landed: affectedCount comes directly from applyAreaFreeze's local affectedRunners.length rather than from the transient state.areaFreezeEffect animation object; runnerRole replaced the runnerTeam guard and is resolved by runnerId lookup rather than by widening event payloads; trace playback was confirmed not to re-drive the engine; and the attempt boundary is defined and tested (counters survive resetRound, reset on initializeMatch and initializeDisplayState). Thirteen tests, four of which drive the real engine through runGuidedLevelWithSolution — including a deliberately engineered ineffective freeze at Manhattan distance 5 against a radius of 2 — plus a clean-attempt negative control. One event-contract change is recorded: runner.actionResolved now carries a conditional affectedCount, documented in docs/subsystems/turn-engine.md. 608/608 unit tests, clean build, lint:levels clean except the two known optional-lab untiered warnings. No criteria authored, no star metadata touched, no export or ledger fields added."
depends_on: [plan-113]
