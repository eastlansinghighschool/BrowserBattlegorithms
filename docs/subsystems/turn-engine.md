...
- If no legal displacement cell exists anywhere on the board, `reconcileFlagHomeOccupancy()` leaves the runner in place and logs a diagnostic via `console.warn` rather than throwing — reconciliation must never add a new engine-halting path, which is exactly the failure class Plan 78 exists to remove. Given board sizes and the one-barrier-per-runner rule, this should not occur on any authored map; if it does, the leftover state is the same mild runner-on-own-loose-flag oddity reconciliation normally prevents, and is caught by `invariants.js` (a test-time check, not a live runtime guard) if it ever occurs.
