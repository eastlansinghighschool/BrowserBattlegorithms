...
- `aria-narration.spec.js` and `key-capture-passthrough.spec.js` remain in smoke because they cover student-facing announcement and keyboard-routing contracts. Note on timing contracts: `p5.noLoop()` prevents scheduling further frames but does not cancel an already-queued `requestAnimationFrame` callback in the browser event loop. Asserting on transient turn-engine state (such as `queuedActionForCurrentRunner`, which is consumed and cleared on the next processed frame) races the render loop by construction. Any browser test that dispatches input and then waits on intermediate engine state must drain the in-flight frame after `noLoop()` (e.g., `await page.evaluate(() => new Promise((resolve) => requestAnimationFrame(() => resolve())))`) before dispatching input (see `plan-122`).
