...
status: complete
resolution: "One criterion authored on one level, which is the honest yield. no-wasted-resource is registered and fail-closed on absent, null, or non-finite counters; show-what-you-know carries it at turnPar 41. The falsifiable pair was re-run independently by the orchestrator: the reference solution passes 35/41 for 3 stars with mastery true, and the degraded fixture passes 37/41 for 2 stars with mastery false -- both beat par, so the criterion alone separates the tiers. Twenty-five of twenty-six candidates are rejected with stated reasons, including the five that turned on an empirical claim, which were simulated rather than argued. Amendment 01 dropped no-collision entirely: runner_collision_bounce fires only when the target cell holds a same-team or already-frozen runner, so the counter measures ally obstruction rather than enemy avoidance, the gate's rationale for admitting it was wrong, and registering a misnamed id no level uses would have been worse than leaving it out. The map-blockage exclusion is now evidenced rather than reasoned: show-what-you-know's reference solution reads mapBlockageBounces 1, so folding it in would have failed the only authorable level on its own criterion -- the exact defect that disqualified bughunt-28. Repair 01 was required after the authorized ride-along extraction silently changed four tags from value to field on battlegorithms_value_compare, which Blockly discards, un-idling the support allies and leaving a green test with a false premise; the fixture is now byte-identical to its source and the test asserts its own premise plus a zero-warning check, verified by the orchestrator to catch the corrupted version on three of four assertions. 613/613 unit tests, clean build, lint:levels clean except the four known untiered warnings."
depends_on: [plan-116]
