...
- Expected artifacts:
  - narrowly scoped current-capability upgrades, only after the recovery gate
...
| `commit-discipline` | core / managed prose | behind: 2.0.0 < 2.1.0; required markers absent from packet guidance and design-review prompt | upstream marker blocks and each local target-file diff | merge only the 2.1 managed prose into every required surface, then marker-recheck | required |
| `advisor-consultation` | core / managed prose | diverged: manifest says adopted but the required packet-guidance marker probe failed | manifest and expected marker locations | repair the managed-prose surface and make the manifest truthful after its probe passes | required |
...
- `docs/agent-starting-prompts/`, `docs/packet-creation-guidance.md`, `docs/development/packet-template.md`, `docs/workflows/packet-tracking-system.md`
...
1. Run the fresh tracked-mode Bootstrap audit and reconcile differences from the preflight docket.
2. For owner-approved current deltas only:
...
3. Update the adoption manifest only for capabilities verified as current, record the environment-derived audit date, and write the progress report.
...
- `docs/packet-creation-guidance.md`, `AGENTS.md`, and `advisor-capable-providers.json` only where the approved ledger requires them
- `reports/development/plan-117-bootstrap-capability-recovery-and-recency/progress.md`

## Work Plan

1. Run `node scripts/dev/plan-status.js check plan-117`; inspect the tree and Plan 115's completed handoff.
2. Run the live Bootstrap audit in tracked mode. If it differs materially from this packet's evidence, stop and amend the triage rather than applying stale recommendations.
3. Present the capability triage and wait for owner approval.
4. Apply approved upgrades in dependency order: configurable prompt changes first; verbatim/tooling and marker-managed blocks next; then truthful manifest changes.
...
### 2. Current audit and version deltas
...
### 4. Prompt and managed-prose updates
...
### 5. Manifest honesty and reporting
