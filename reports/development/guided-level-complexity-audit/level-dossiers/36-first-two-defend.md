# Guided Level Dossier: Level 31: First Two Defend
...
- project: `team-strategy-script step 3`

## Curriculum Row
- focus: First two defend
- new vocabulary: grouping allies by range
- new Blockly: index < 2
- assumptions: Levels 29-30
...
## Lesson Copy
- objective: Teach range checks on runner index so two allies take one job and the third takes another.
- intro: Now the team has three program-controlled allies. Index < 2 is a clean way to group the first two together and move them out of the way, while the third ally runs forward.
- tips:
  - Stay Still can remove a barrier directly in front — remember that from an earlier level?
- tutorial steps:
  - 1. Index Ranges Create Teams (level-29-range)
    - body: Index < 2 can group the first two allies together while index 2 heads forward. That keeps the shared script simple and readable.
    - demo Blockly: not found
  - 2. Three Allies, One Program (level-29-three-allies)
    - body: Two allies need to clear space so the third runner can finish the puzzle.
    - demo Blockly: not found
...
- objective: runner runner_1_AI_AllyP1_3 reaches (4, 4)
...
- jail cells: none
- flags: not found
- barriers: (5, 4) owner phase8_first_two_defend_wall
...
- player runner 1 (runner_1_AI_AllyP1) slot ally 0 at (2, 4); control ally; frozen no; enemy flag no
  - play direction: 1; home side: left; can jump: yes; can place barrier: no
- player runner 2 (runner_1_AI_AllyP1_2) slot ally 1 at (3, 4); control ally; frozen no; enemy flag no
  - play direction: 1; home side: left; can jump: yes; can place barrier: no
- player runner 3 (runner_1_AI_AllyP1_3) slot ally 2 at (1, 4); control ally; frozen no; enemy flag no
  - play direction: 1; home side: left; can jump: yes; can place barrier: no
...
- Project step fixture 3: `tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-03.xml`
  ...
  - distinct block types: 7 types: `battlegorithms_if_boolean_else`, `battlegorithms_move_forward`, `battlegorithms_move_up_screen`, `battlegorithms_on_each_turn`, `battlegorithms_value_compare`, `battlegorithms_value_number`, `battlegorithms_value_runner_index`
...
## Facts Only
- shared workspace project team-strategy-script step 3
- concept focus present: First two defend
- starter XML contains 1 blocks
- project fixture XML present (2 files)
- toolbox exposes 48 authored block types

## Validation Pointers
- Readiness command: npm run level:readiness -- --level first-two-defend --json
- Linter command: npm run lint:levels
- Project step fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-03.xml
- Project final fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/final.xml
