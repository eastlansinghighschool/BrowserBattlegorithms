...
9. **V2 Export Payload Shape & Boundary Rules (Plan 108)**:
   ...
   - **Plan 91 / Plan 110 / Plan 111 Star Evaluation & Durable Receptacle**:
     ...
     - **2-Star Max & Concept-Mandatory Levels**: Levels where the core concept is mandatory to pass (such as movement-helpers pilot levels 12–14) do not author a `masteryCriterionId` (documented as "no honest criterion — 2-star max" per decision log 2026-07-22), capping stars at 2 without code-golf block-budget enforcement. S12 fully protected levels (e.g. `move-toward-flag`) carry no star metadata and remain pass-star-only (max 1 star).
     - **Cumulative Tiers**: Stars are cumulative — `starsEarned` is 3 only when both `parBeaten` and `masteryAchieved` hold, and 2 only when `parBeaten` holds. A mastery-meeting but slower-than-par run earns 1 star with `masteryAchieved: true` recorded (decision log 2026-07-22).
