# Guided Level Dossier: Prediction: Role Split
...
- source file: `src/config/levels/phases/advanced-teamplay/prediction-31-index-role-split.js`
- project: not applicable

## Curriculum Row
- focus: Runner index prediction
- new vocabulary: commit to which ally takes the first shared-program job
- new Blockly: prediction checkpoint; no new Blockly idea
- assumptions: Levels 29-31
...
## Lesson Copy
- objective: Predict which runner takes the forward job when one program runs on both allies.
- intro: The shared program now runs on two allies. Pick which runner takes the first action, then run and compare the outcome.
- tips: not found
- tutorial steps:
  - 1. Trace the Runner Index (prediction-31-intro)
...
- distinct block types: 7 types: `battlegorithms_if_boolean_else`, `battlegorithms_move_forward`, `battlegorithms_on_each_turn`, `battlegorithms_stay_still`, `battlegorithms_value_compare`, `battlegorithms_value_number`, `battlegorithms_value_runner_index`
- first-action-only risk markers: none found
### Demo XML
- status: not found
### Reference XML
- source: `tests/unit/fixtures/guided-reference-solutions/prediction-31.xml`
- present: yes
- metrics: blocks 7; distinct types 7; actions 2; conditions 1; boolean/comparison/value 3; max depth 4; decision points 2; runner index 1; resource readiness 0
- distinct block types: 7 types: `battlegorithms_if_boolean_else`, `battlegorithms_move_forward`, `battlegorithms_on_each_turn`, `battlegorithms_stay_still`, `battlegorithms_value_compare`, `battlegorithms_value_number`, `battlegorithms_value_runner_index`
- first-action-only risk markers: none found
...
- concept focus present: Runner index prediction
- starter XML contains 7 blocks
- reference XML contains 7 blocks
- toolbox exposes 25 authored block types

## Validation Pointers
- Readiness command: npm run level:readiness -- --level prediction-31 --json
- Linter command: npm run lint:levels
- Reference fixture: tests/unit/fixtures/guided-reference-solutions/prediction-31.xml
