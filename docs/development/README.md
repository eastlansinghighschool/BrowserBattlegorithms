...
| [Plan 35](plan-35-narration-event-log-foundation.md) | complete | Per-turn append-only event log on `state` with 9 v1 event kinds (`turn.started`, `runner.actionChosen`, `runner.actionResolved`, `runner.blockedOrBounced`, `flag.pickedUp`, `flag.dropped`, `team.scored`, `resource.unavailable`, `level.result`). Engine becomes a passive producer; no consumers in this packet. Foundation for narration, classifier, replay, and usage enrichment. |
...
| [Plan 42](plan-42-bug-hunt-guided-levels.md) | ready | Add one guided bug hunt level before each synthesis challenge so students practice tracing and repairing plausible broken Blockly programs before open-ended challenge work. |
