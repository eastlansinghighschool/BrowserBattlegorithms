...
| Collision/waste event tracking for star criteria | deferred | Plan-113 gate (decision log 2026-07-22): no-collision and no-wasted-resource were dropped because no level-run counters exist at level end. Building per-run collision and wasted-resource tracking (likely from the Plan 35 event log) unlocks honest star-3 criteria on scrimmage and resource levels beyond Phase 6's both-allies-active. |
| Capstone step-9 fixture tuning (advanced-scrimmage) | deferred | Documented debt (tests/unit/guided-project-solutions.test.js:31-45): the team-strategy-script step-9 checkpoint fixture fails the current NPC layout (turn 56 vs limit 55). Needs a tuned, reliable capstone checkpoint solution; until then advanced-scrimmage is pass-star-only. |
