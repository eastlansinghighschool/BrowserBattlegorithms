...
9. **V2 Export Payload Shape & Boundary Rules (Plan 108)**:
   ...
   - **Plan 91 / Plan 110 Rewrite-Aware Fields Receptacle**: The per-level ledger rollup entry (`createGuidedLevelRollupEntry`) contains optional, additive schema slots for campaign-rewrite fields (`arcId`, `arcStageIndex`, `arcStageCount`, `boardDynamicsTier`, `bestiaryEncounterIds`, `starsEarned`, `parBeaten`, `turnPar`, `masteryAchieved`, `masteryCriterionId`, `filmReviewSummary`). Unset fields remain absent from serialized output objects rather than emitting `null` noise. Absence is tolerated on read and write across all export schema versions. Closed vocabulary for `masteryCriterionId` (`concept-used`, `no-wasted-resource`, `both-allies-active`, `no-collision`, `under-block-budget`) and board tiers (`static-prop`, `background-motion`, `timing-threat`, `collision-threat`, `scrimmage-threat`) follow Plan 85 charter rules — but these vocabularies are **documentary only at this layer**: the receptacle accepts any non-empty string, and validation/enforcement is owned by the producing packets (plan-96 and later). `bestiaryEncounterIds` accepts either an array of archetype ids or a compact object summary (Plan 91's "equivalent cheap encounter signal"); the producing packet picks one and documents it. Unknown entry fields are ignored on hydration (dropped, not rejected) — pinned by test.
