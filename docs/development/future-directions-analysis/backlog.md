...
| --- | --- | --- |
| Student visual customization (skins, tilesets, theme) | **Plan 123 (charter, draft)** | Owner brain-dump 2026-09-09. Per-student icon/tileset choice, animation options, light/dark. Orchestration finding: four of the five needed pieces already exist — `resolveRunnerDisplayEmoji()` in `src/render/runnerVisuals.js` is a 14-line pure table lookup and the single appearance seam; `src/render/effects.js` already carries a state-overlay layer (frozen glow, area-freeze pulse) independent of the glyph; the settings modal already has a five-toggle visual/motion group; and `src/ui/preferences.js` is exception-safe since plan-118. The missing piece is the **contract**, not the art. Tiering is counterintuitive: alternate glyph sets are the easy win, light/dark is the *hardest* (CSS + p5 canvas color constants + Blockly's own theme API, all with contrast obligations), sprite sheets are deferred behind licensing, ARIA, and load-path risk. Governing rule to ratify: **base glyph = identity, overlay = state** — which lets alternate sets supply 6 slots instead of 12. Next artifact: a charter-shaped decision packet, no code. |
