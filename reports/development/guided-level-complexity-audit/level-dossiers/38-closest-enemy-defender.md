# Guided Level Dossier: Level 33: Closest Enemy Defender
...
- project: `team-strategy-script step 5`

## Curriculum Row
- focus: Closest enemy defender
- new vocabulary: split attack/defense jobs
- new Blockly: index + closest enemy
- assumptions: Levels 23, 29-32
...
## Lesson Copy
- objective: One ally attacks while another uses closest-enemy targeting as a defender.
- intro: This is the first advanced level where one ally chases the goal and another reacts to live enemies that have already crossed onto your side. The shared script is starting to split attack and defense.
- tips: not found
- tutorial steps:
  - 1. Split The Team Jobs (level-31-split)
    - body: Use runner index to make the first ally attack and the second react to the closest enemy. Each ally is still running the same code, just with a different role.
    - demo Blockly: not found
  - 2. Defend Your Side First (level-31-pressure)
    - body: The defender’s job starts on your side of the field while the attacker keeps advancing.
    - demo Blockly: not found
...
## Runner Facts
- player runner 0 (runner_1_HumanP1) slot human at (1, 7); control human; frozen no; enemy flag no
...
- player runner 1 (runner_1_AI_AllyP1) slot ally 0 at (1, 4); control ally; frozen no; enemy flag no
  - play direction: 1; home side: left; can jump: yes; can place barrier: no
- player runner 2 (runner_1_AI_AllyP1_2) slot ally 1 at (6, 2); control ally; frozen no; enemy flag no
  - play direction: 1; home side: left; can jump: yes; can place barrier: no
- opponent runner 0 (runner_2_Npc1) slot npc 0 at (4, 2); control npc; frozen no; enemy flag no
  - play direction: -1; home side: right; can jump: yes; can place barrier: no
- opponent runner 1 (runner_2_Npc2) slot npc 1 at (3, 2); control npc; frozen no; enemy flag no
  - play direction: -1; home side: right; can jump: yes; can place barrier: no
...
- Project step fixture 5: `tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-05.xml`
...
## Facts Only
- shared workspace project team-strategy-script step 5
- concept focus present: Closest enemy defender
- starter XML contains 1 blocks
- project fixture XML present (2 files)
- toolbox exposes 48 authored block types

## Validation Pointers
- Readiness command: npm run level:readiness -- --level closest-enemy-defender --json
- Linter command: npm run lint:levels
- Project step fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/step-05.xml
- Project final fixture: tests/unit/fixtures/guided-project-solutions/team-strategy-script/final.xml
