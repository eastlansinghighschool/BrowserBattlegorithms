...
| [Plan 56](plan-56-cell-inspector-tooltip.md) | complete | DOM hover/tap cell inspector overlay layered over the p5 canvas. Shows coordinate plus all entities on the cell (runners with side/control/index/frozen/flag-carry/resources, flags, barriers, level goal, flag bases). Allies and enemies symmetric. Hover + tap-to-pin + ESC. `aria-live` mirror for screen readers. Always on; no toggle. |
| [Plan 57](plan-57-settings-gear-panel.md) | ready | Gear-icon settings modal that consolidates every UI-level preference (Turn Log, Coaching Mode, Voice Narration, Sound, plus new low-motion override, runner movement animations, runner jumping animations, show frozen badges, show runner-index badges). Global `localStorage` persistence under `bba:settings:` prefix. Focus trap + ESC. Broadens `prefersReducedMotion(state)` across every motion site. Blockly sounds remain reload-required. |
| [Plan 58](plan-58-runner-index-badges.md) | ready | Small team-tinted numeric badge over every runner (allies AND enemies) when the Plan 57 toggle is on. Component takes `(side, index)` internally for future labeling-scheme extension. Lower-left placement to avoid collision with the frozen badge upper-left. Default off. Depends on Plan 57. |
