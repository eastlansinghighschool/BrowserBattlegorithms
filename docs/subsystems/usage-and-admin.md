...
9. **V2 Export Payload Shape & Boundary Rules (Plan 108)**:
   ...
   - **Plan 91 / Plan 110 / Plan 111 Star Evaluation & Durable Receptacle**:
     ...
     - **Pure Evaluator & Wiring (`src/core/starEvaluation.js`)**: Evaluated synchronously on level completion through the real `endLevel` -> `recordLevelEnded` path via `evaluateLevelStars`. End-of-level `details` passed to `recordLevelEnded` and `evaluateLevelStars` additively carry the four Plan 116 attempt counters (`runnerCollisionBounces`, `mapBlockageBounces`, `resourceUnavailableAttempts`, `ineffectiveFreezeUses`) for consumption by mastery criteria evaluators.
     ...
     - **Criterion Registry & Closed Vocabulary**: Mastery criteria evaluation maps `masteryCriterionId` through a criterion registry (`registerCriterionEvaluator`). Closed vocabulary follows Plan 85 S6 (`concept-used`, `no-wasted-resource`, `both-allies-active`, `no-collision`, `under-block-budget`). Unknown or un-registered criteria fail open safely (`masteryAchieved = false`, no errors thrown). Evaluators are checkable from data available at level end (`runnerActionHistory`, turn details):
        - `both-allies-active`: checks all player ally runners executed at least one action.
        - `no-wasted-resource` (Plan 124): mechanically checks `details.resourceUnavailableAttempts === 0 && details.ineffectiveFreezeUses === 0`. Absent or non-numeric counters fail closed (`return false`). Authored on Challenge 22 (`show-what-you-know`).
        - `no-collision` (Plan 124 Amendment 01): not registered. Engine analysis confirmed `runner_collision_bounce` fires only on same-team or already-frozen runners (live enemy contact resolves as tag/capture without bounce), so the counter measures ally obstruction rather than enemy collision; no level discriminates on it.
      - **2-Star Max & Concept-Mandatory Levels**: Levels where the core concept is mandatory to pass (such as movement-helpers pilot levels 12–14) do not author a `masteryCriterionId` (documented as "no honest criterion — 2-star max" per decision log 2026-08-05), capping stars at 2 without code-golf block-budget enforcement. 25 of 26 candidates inspected in Plan 124 remain 2-star max. S12 fully protected levels (e.g. `move-toward-flag`) carry no star metadata and remain pass-star-only (max 1 star).
