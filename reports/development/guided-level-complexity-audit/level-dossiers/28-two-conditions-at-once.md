# Guided Level Dossier: Level 25: Two Conditions At Once
...
- project: `strategy-brain step 3`

## Curriculum Row
- focus: Two conditions at once
- new vocabulary: two truths required for the same strategy
- new Blockly: AND
...
## Lesson Copy
- objective: Use AND so freeze only happens when the enemy is close and the team power is ready again.
- intro: The same Strategy Brain can wait for two truths at once. Here it should only spend the freeze when both the distance and readiness checks say to act.
- tips: not found
- tutorial steps:
  - 1. Both Must Be True (level-23-and)
    - body: AND is useful for a timed power: close enough to matter, and ready to use again. That is how the shared program decides when to spend its freeze.
    - demo Blockly: not found
  - 2. Freeze Then Continue (level-23-lane)
    - body: After the freeze is spent, the ally should keep moving toward the flag. The Strategy Brain should not get stuck on the special action.
    - demo Blockly: not found
...
- Project step fixture 3: `tests/unit/fixtures/guided-project-solutions/strategy-brain/step-03.xml`
...
## Facts Only
- shared workspace project strategy-brain step 3
- concept focus present: Two conditions at once
- starter XML contains 1 blocks
- project fixture XML present (2 files)
- toolbox exposes 33 authored block types

## Validation Pointers
- Readiness command: npm run level:readiness -- --level two-conditions-at-once --json
- Linter command: npm run lint:levels
- Project step fixture: tests/unit/fixtures/guided-project-solutions/strategy-brain/step-03.xml
- Project final fixture: tests/unit/fixtures/guided-project-solutions/strategy-brain/final.xml
