...
| [Plan 55](plan-55-level-result-invariant-at-game-over.md) | ready | Enforce the engine invariant that `mainGameState === GAME_OVER` always coincides with a defined `activeLevelResult`. Closes the Level 29 wrong-runner-scoring lockup (surfaced by 2026-05-18 pilot) and any similar class of multi-ally runner-specific win conditions. Distinct from Plan 28's `PROCESSING_ACTION` recovery — same symptom, different root cause. |
